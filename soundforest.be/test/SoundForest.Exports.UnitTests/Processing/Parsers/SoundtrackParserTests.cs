...
using Xunit;
using IDocumentLoader = SoundForest.Exports.Processing.Infrastructure.Parsers.IDocumentLoader;
...
public sealed class SoundtrackParserTests
{
    ...
    private static IHtmlDocument[] ValidHtmlDocuments = new IHtmlDocument[] {
        new HtmlParser().ParseDocument(ValidHtml)
    };
    ...
    internal async Task SoundtrackParser_Succeeds(
        ...
        Mock<IDocumentLoader> documentLoader,
        string id)
    {
        // Arrange
        documentLoader
            .Setup(d => d.LoadDocumentsAsync(It.IsAny<IEnumerable<string>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(ValidHtmlDocuments);

        var sut = new SoundtrackParser(logger, documentLoader.Object);
